...
volatile bool okToSleep = false;
volatile bool screenIsOf = false;
...
void handleReedSwitch() {
  ...
  if (currentState == HIGH) {  // La pin est passée à HIGH
...
void goToSleep(String origin = "...") {
  Serial.println("Prep to sleep...");
  Serial.println(origin);
  delay(5);  //lui permet d'avoir le temps de finir d'écrire sur le serial.
  ...
  if (digitalRead(reedSwitchPin) == HIGH) {  // Magnet is PROCHE -> pin is HIGH
  ...
  } else {  // Magnet is LOIN -> pin is LOWoj
    ...
    okToSleep = false; //2025-06-25 13:09:23 - Important, prevent sleep
  }
